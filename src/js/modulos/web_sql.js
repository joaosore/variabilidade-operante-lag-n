...
let db = openDatabase("Experimento_Reniel", "1.0", "Teste Web SQL Database", 200000);
let id;
...
function version(version) {
    db.transaction(function(transaction){
        ...
        transaction.executeSql("SELECT * FROM version WHERE version = '"+version+"'", [], function(transaction, result) {
            ...
            {
                transaction.executeSql('INSERT INTO version (version) VALUES ("'+version+'")');
                deleteTable();
                create();
            }
        });
...
{
	db.transaction(function(transaction){
        transaction.executeSql("CREATE TABLE IF NOT EXISTS leg_0 (nome TEXT, leg TEXT, combinacoes TEXT, tempo TEXT)");
        transaction.executeSql("CREATE TABLE IF NOT EXISTS leg_2 (nome TEXT, leg TEXT, combinacoes TEXT, tempo TEXT)");
        transaction.executeSql("CREATE TABLE IF NOT EXISTS leg_5 (nome TEXT, leg TEXT, combinacoes TEXT, tempo TEXT)");
        transaction.executeSql("CREATE TABLE IF NOT EXISTS leg_8 (nome TEXT, leg TEXT, combinacoes TEXT, tempo TEXT)");
	    transaction.executeSql("CREATE TABLE IF NOT EXISTS AC (nome TEXT, leg TEXT, combinacoes TEXT, tempo TEXT)");
	});
}
...
export function select(Table, Nome) {
    db.transaction(function(transaction) {
        transaction.executeSql("SELECT * FROM "+Table+" WHERE nome = '"+Nome+"'", [], function(transaction, result) {
...
{
    db.transaction(function(transaction) {
        transaction.executeSql("SELECT * FROM leg_0", [], function(transaction, result) {
            for(var i = 0; i < result.rows.length; i++){
                var line = result.rows.item(i);
    ...
    db.transaction(function(transaction) {
        transaction.executeSql("SELECT * FROM leg_2", [], function(transaction, result) {
            for(var i = 0; i < result.rows.length; i++){
                var line = result.rows.item(i);
                INTEGRANTES.push(line.nome);
            }
    ...
    db.transaction(function(transaction) {
        transaction.executeSql("SELECT * FROM leg_5", [], function(transaction, result) {
            for(var i = 0; i < result.rows.length; i++){
                var line = result.rows.item(i);
                INTEGRANTES.push(line.nome);
            }
    ...
    db.transaction(function(transaction) {
        transaction.executeSql("SELECT * FROM leg_8", [], function(transaction, result) {
            for(var i = 0; i < result.rows.length; i++){
                var line = result.rows.item(i);
                INTEGRANTES.push(line.nome);
            }
    ...
    db.transaction(function(transaction) {
        transaction.executeSql("SELECT * FROM AC", [], function(transaction, result) {
            for(var i = 0; i < result.rows.length; i++){
                var line = result.rows.item(i);
                INTEGRANTES.push(line.nome);
            }
...
export function insert(LEG, nome, leg_0_combinacoes, leg_0_tentativas_unidades, tempo)
{
    console.log('------------------------------------------------------');
    console.log('Insert');
    console.log('LEG', LEG);
    ...
    console.log('------------------------------------------------------');
	db.transaction(function(transaction){
	    transaction.executeSql('INSERT INTO '+LEG+' (nome, leg, combinacoes, tempo) VALUES (?, ?, ?, ?)', [nome, leg_0_combinacoes, leg_0_tentativas_unidades, tempo],
            function(tx, results){
            	id = results.insertId;
            }
        );
...
export function update(LEG, leg_0_combinacoes, leg_0_tentativas_unidades, tempo, ID)
{
	db.transaction(function(tx) {
        tx.executeSql('update '+LEG+' set leg=?, combinacoes=?, tempo=? where id=?', [leg_0_combinacoes, leg_0_tentativas_unidades, tempo, ID], function(transaction, result) {
            console.log(result);
